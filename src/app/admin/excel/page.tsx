...
export default function ExcelPage() {
  ...
  async function commitImport() {
    ...
    const data = await res.json();
    setImporting(false);
    setResult(`${data.created} new customer(s) added, ${data.updated} existing customer(s) updated.`);
    setRows(null);
  }
  ...
  const createCount = rows?.filter((r) => r.errors.length === 0 && !r.isUpdate).length ?? 0;
  const updateCount = rows?.filter((r) => r.errors.length === 0 && r.isUpdate).length ?? 0;

  return (
    <div>
      <h1 className="font-display text-2xl font-bold text-navy-900">Excel Import / Export</h1>
      <p className="mt-1 text-sm text-gray-500">Import new customers or update existing ones from a spreadsheet, or export the current list.</p>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        <div className="rounded-md border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-500">
            ...
            If an Account Number already exists, that customer's details will be updated instead of creating a duplicate.
          </p>
          <input
            type="file"
            accept=".xlsx"
        ...
        <div className="mt-6 rounded-md border border-gray-100 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-lg font-semibold text-navy-900">
              Preview — {createCount} new, {updateCount} update{updateCount === 1 ? "" : "s"}, {rows.length - validCount} error(s)
            </h2>
                  ...
                  <tr key={r.rowNumber} className={`border-b border-gray-50 ${r.errors.length ? "bg-danger/5" : ""}`}>
                    ...
                    <td className="px-3 py-2">
                      {r.errors.length ? (
                        ...
                        <span className="text-xs font-medium text-gold-500">Will update existing customer</span>
                      ) : (
                        <span className="text-xs font-medium text-success">New customer</span>
                      )}
                    </td>
                  </tr>
                ))}
